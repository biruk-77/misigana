...
const Hero: React.FC<HeroProps> = ({ onEnter }) => {
  return (
    <div className="relative h-[100dvh] w-full flex flex-col items-center justify-center overflow-hidden bg-champagne p-4 scene-3d preserve-3d">
      ...
      <div className="relative z-20 flex flex-col items-center text-center max-w-6xl mx-auto w-full space-y-4 sm:space-y-6">
        ...
        >
          <div className="w-px h-8 sm:h-12 bg-gradient-to-b from-transparent to-gold mb-2" />
          <div className="flex items-center gap-4">
            <Heart size={12} className="text-gold fill-gold/20" />
            <span className="text-[8px] sm:text-[10px] font-black uppercase tracking-[0.6em] sm:tracking-[0.8em] text-gold/60">A Sovereign Sanctuary</span>
            <Heart size={12} className="text-gold fill-gold/20" />
          </div>
        </motion.div>
        ...
        >
          <div className="absolute -top-8 -left-8 opacity-10 animate-float-slow pointer-events-none hidden sm:block">
            <Sparkles size={100} className="text-gold" />
          </div>

          <h1 className="text-5xl sm:text-7xl md:text-8xl font-display italic font-black text-espresso tracking-tighter leading-[0.8] py-1 select-none">
            Biruk <br />
            <span className="luxury-text-gradient py-0 inline-block">&</span> <br />
            Misgana
          </h1>

          <div className="absolute -bottom-4 -right-2 pointer-events-none group-hover:scale-110 transition-transform duration-1000">
            <div className="bg-white/95 backdrop-blur-2xl p-2 sm:p-4 rounded-2xl shadow-xl border border-gold/10 rotate-12">
              <Star size={20} className="text-gold fill-gold/5 animate-pulse" />
            </div>
          </div>
        </motion.div>
